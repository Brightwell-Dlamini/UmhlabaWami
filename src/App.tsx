...
export default function App() {
  ...
  const renderDashboard = () => {
    if (!currentUser) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[40vh] text-center p-8">
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-3">Sign in to open the operations dashboard.</p>
          <button type="button" onClick={() => setIsLoginOpen(true)} className="px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-semibold">
            Sign In
          </button>
        </div>
      );
    ...
    if (sidebarActiveTab === 'ai_assist') return <AiAssistView onViewTicket={(id) => setSelectedTicketId(id)} />;
    if (sidebarActiveTab === 'permissions') return <PermissionsView />;
    if (sidebarActiveTab === 'compliance_audit' || sidebarActiveTab === 'audit_logs') return <ComplianceAuditView />;
    ...
    if (sidebarActiveTab === 'maintenance_ops') return <MaintenancePortal onViewTicket={(id) => setSelectedTicketId(id)} />;
    ...
    if (
      ['finance', 'finance_overview', 'rent_roll', 'expenses_ledger', 'transactions', 'financial_requests', 'finance_documents'].includes(
        sidebarActiveTab
      )
    )
      return <FinancePortal initialTab={sidebarActiveTab} />;
    if (
      ['super_admin', 'super_overview', 'super_approvals', 'super_organizations', 'super_listings', 'db_backup'].includes(
        sidebarActiveTab
      )
    )
    ...
    if (currentUser.role === 'property_manager' || currentUser.role === 'admin')
      return <CentrePulseView onViewTicket={(id) => setSelectedTicketId(id)} onNavigate={handleTabChange} />;
    if (currentUser.role === 'maintenance') return <MaintenancePortal onViewTicket={(id) => setSelectedTicketId(id)} />;
    if (currentUser.role === 'finance') return <RentRollArrearsView />;
    if (currentUser.role === 'super_admin') return <SuperAdminPortal initialTab={sidebarActiveTab} />;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      ...
      <Navbar
        ...
        onSwitchViewMode={(mode) => {
          if (mode === 'dashboard') goToDashboard();
          else setViewMode('marketplace');
        }}
      ...
      <div className="flex-1 flex flex-col">
        {viewMode === 'marketplace' || !currentUser ? (
