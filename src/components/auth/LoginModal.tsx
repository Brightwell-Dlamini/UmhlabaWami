...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const completeLogin = async (code: string, user: string, pass: string) => {
    ...
    if (!pass.trim()) {
      setErrorMsg('Enter your Supabase Auth password.');
      return;
    }
    setLoading(true);
    try {
      ...
      if (res.success) {
        ...
        setErrorMsg(res.error || 'Login failed. Check organisation code, username, and password.');
      }
    } catch (err) {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Supabase Auth · multi-tenant access</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <form onSubmit={handleLogin} className="space-y-3.5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] text-slate-400">Supabase Auth password</span>
              </div>
              <div className="flex items-center px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-blue-600">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
            <div className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1">
              ...
              <span>Quick-fill username — then enter Auth password</span>
            </div>
